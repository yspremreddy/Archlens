...
ArchLens is an AI Architecture Risk & Compliance Reviewer. These guidelines
apply across the whole codebase, regardless of which part is being worked
on. They exist to keep a system that makes compliance and risk claims
trustworthy, auditable, and safe to operate on content it did not create.
...
2. **Implement only the requested scope.** Don't build ahead of what's
   needed, even if the roadmap describes later phases. If a change implies
   work that spans multiple areas, scope it down explicitly before writing
   code.
...
4. **Test changes.** Every change that can be verified (unit test,
   integration test, manual run, lint) should be verified before it's
   considered done. State plainly when something could not be tested and
   why.
...
- [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) — target system design and
  rationale.
- [`docs/DECISIONS.md`](DECISIONS.md) — decision log (ADR-style).
- [`docs/ROADMAP.md`](ROADMAP.md) — phased build plan.
- [`TODO.md`](../TODO.md) — current, near-term task list.
